      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {reservations.map((res, idx) => (
            <div key={res.id} className="group bg-white dark:bg-[#16171b] border border-gray-100 dark:border-white/5 rounded-[40px] overflow-hidden hover:border-[var(--primary-30)] transition-all duration-500 shadow-sm hover:shadow-2xl">
              <div className="bg-black p-8 relative overflow-hidden">
                <div className="flex justify-between items-start relative z-10">
                  <div className="space-y-2">
                     <p className="text-[10px] font-black text-[var(--primary)] uppercase italic tracking-widest leading-none">Reserva #{res.id.split('-')[0]}</p>
                  ...
                  <div className="pt-2 flex flex-col gap-2">
                    ...
                    {res.ingreso_fecha && res.salida_fecha ? (
                      ...
                      <button 
                         ...
                         className="flex items-center justify-center gap-2 mt-2 py-4 bg-[var(--primary)] hover:bg-white text-black rounded-2xl w-full transition-all group shadow-lg shadow-[var(--primary-10)]"
                      >
          ...
          return (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl animate-in fade-in duration-300">
               <div className="bg-[#16171b] border border-white/10 p-10 rounded-[48px] max-w-sm w-full shadow-2xl relative text-center">
                  ...
                  <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-6 ${isCompleted ? 'bg-emerald-500/10' : 'bg-[var(--primary-10)]'}`}>
                     {isCompleted ? <CheckCircle2 className="w-8 h-8 text-emerald-500" /> : <QrCode className="w-8 h-8 text-[var(--primary)]" />}
                  </div>
                  ...
                  {!isCompleted && (
                      <div className={`mt-8 p-4 rounded-2xl ${hasIngreso ? 'bg-amber-500/10 border border-amber-500/20' : 'bg-[var(--primary-5)] border border-[var(--primary-10)]'}`}>
                          <p className={`text-[8px] font-black uppercase tracking-widest leading-relaxed ${hasIngreso ? 'text-amber-500' : 'text-[var(--primary)]'}`}>
