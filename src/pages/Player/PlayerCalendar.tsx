      ...
      <div className="flex flex-col md:flex-row md:flex-wrap lg:flex-nowrap justify-between items-start md:items-center gap-6">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-club-primary/10 rounded-2xl">
            <CalendarIcon className="w-8 h-8 text-club-primary" />
          </div>
          <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
             <div className="col-span-full py-20 flex flex-col items-center justify-center gap-4">
                <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-club-primary"></div>
          ...
          events.map((event) => (
            <div key={event.id} className="group bg-white dark:bg-[#16171b] border border-gray-100 dark:border-white/5 rounded-[40px] p-8 hover:border-club-primary hover:shadow-2xl transition-all duration-500 relative overflow-hidden">
               {/* Date Badge */}
               <div className="absolute top-0 right-0 p-6">
                  <div className="w-14 h-14 bg-black dark:bg-[#111215] rounded-2xl flex flex-col items-center justify-center shadow-lg group-hover:bg-club-primary transition-colors">
               ...
               <div className="space-y-6 pt-4">
                  <div className="space-y-2">
                    <span className={`px-3 py-1 text-[8px] font-black uppercase tracking-widest rounded-full ${event.isJuego ? 'bg-club-primary/20 text-club-primary' : event.tipo === 'entrenamiento' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'}`}>
                  ...
                  <div className="pt-4 flex gap-2">
                      <Button 
                        ...
                        className="flex-1 py-3 bg-gray-50 dark:bg-white/5 text-[9px] font-black uppercase tracking-widest italic rounded-xl border border-gray-100 dark:border-white/5 hover:bg-club-primary hover:text-black transition-all"
                      >
      ...
      >
        <div className="space-y-8">
           ...
           {planning?.objetivos?.length > 0 && (
             <div className="bg-club-primary/5 p-6 rounded-[32px] border border-club-primary/10 space-y-4">
                <div className="flex items-center gap-2">
                   <div className="p-2 bg-club-primary text-black rounded-lg"><Info size={14} /></div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                   {planning.objetivos.map((obj: any, idx: number) => (
                     <div key={idx} className="flex items-center gap-3 bg-white dark:bg-white/5 p-3 rounded-2xl border border-gray-100 dark:border-white/5">
                        <div className="w-1.5 h-1.5 rounded-full bg-club-primary"></div>
           ...
           {playerAttendance && (
             <div className="bg-white dark:bg-white/5 p-6 rounded-[32px] border border-gray-100 dark:border-white/10 space-y-6">
                ...
                {playerAttendance.estado === 'presente' && playerAttendance.puntaje_total > 0 && (
                  <div className="space-y-4">
                    <div className="flex items-center gap-4">
                       ...
                       <span className="text-2xl font-black text-club-primary italic leading-none">{playerAttendance.puntaje_total}%</span>
                    </div>
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        {playerAttendance.evaluaciones.map((ev: any, idx: number) => (
                          <div key={idx} className="flex items-center justify-between p-2 rounded-xl bg-gray-50 dark:bg-black/20">
                            ...
                            <span className="text-[9px] font-black text-club-primary">{ev.puntaje}/5</span>
                          </div>
                        ))}
                      </div>
                    )}
                ...
                {playerAttendance.notas && (
                  <div className="p-4 bg-club-primary/5 border border-club-primary/10 rounded-2xl">
           ...
           <div className="pt-6 border-t border-gray-100 dark:border-white/5 flex gap-4">
              ...
              {!confirmedEvents.has(selectedEvent?.id) ? (
                <Button 
                  ...
                  disabled={confirming}
                  className="flex-1 py-4 bg-club-primary text-black text-[10px] font-black uppercase tracking-widest italic rounded-2xl shadow-xl shadow-club-primary/20 hover:scale-105 transition-all"
                >
