      ...
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black tracking-tighter text-gray-900 dark:text-white flex items-center gap-3 uppercase italic">
            <Trophy className="text-[var(--primary)]" size={32} /> Gestión de Juegos
          </h1>
        ...
        <Button onClick={() => setIsModalOpen(true)} className="bg-[var(--primary)] text-black hover:bg-[#aacc00] rounded-2xl font-black px-8 py-6 uppercase italic text-xs tracking-widest shadow-xl shadow-[var(--primary-10)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {friendlies.length === 0 ? (
            ...
            friendlies.map(match => (
              <div key={match.id} className={`bg-white dark:bg-[#16171b] border border-gray-100 dark:border-white/5 rounded-3xl p-6 relative overflow-hidden ${match.estado === 'Played' ? 'border-l-4 border-l-emerald-500' : 'border-l-4 border-l-[var(--primary)]'}`}>
                ...
                <div className="flex gap-3">
                   <Button 
                    className="flex-1 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white hover:bg-[var(--primary)] hover:text-black hover:border-transparent transition-all font-bold text-xs"
                    onClick={() => {
         ...
         <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <div>
                  ...
                  <input className="w-full h-12 bg-gray-50 dark:bg-white/5 rounded-xl px-4 text-sm font-medium border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[var(--primary)]" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} placeholder="Cancha Principal" />
               </div>
               <div className="grid grid-cols-2 gap-4">
                  <div>
                     <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Fecha</label>
                     <input type="date" className="w-full h-12 bg-gray-50 dark:bg-white/5 rounded-xl px-4 text-sm font-medium border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[var(--primary)]" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} />
                  </div>
                  <div>
                     <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Hora</label>
                     <input type="time" className="w-full h-12 bg-gray-50 dark:bg-white/5 rounded-xl px-4 text-sm font-medium border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[var(--primary)]" value={formData.time} onChange={e => setFormData({...formData, time: e.target.value})} />
                  </div>
               </div>
            </div>
                  ...
                  <div className="space-y-3">
                     {formData.teamA.players.map((p, i) => (
                        <div key={i} className="flex gap-2 items-center">
                           <input className="w-16 h-10 bg-white dark:bg-[#16171b] rounded-lg px-2 text-center text-sm font-bold border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white focus:outline-none focus:border-[var(--primary)]" placeholder="00" value={p.number} onChange={e => handlePlayerChange('teamA', i, 'number', e.target.value)} />
                           <input className="flex-1 h-10 bg-white dark:bg-[#16171b] rounded-lg px-3 text-sm font-medium border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white focus:outline-none focus:border-[var(--primary)]" placeholder="Nombre Jugador" value={p.name} onChange={e => handlePlayerChange('teamA', i, 'name', e.target.value)} />
                     ...
                     <button type="button" onClick={() => handleAddPlayer('teamA')} className="text-xs font-bold text-[var(--primary)] hover:text-[#aacc00] flex items-center gap-1 mt-2">
