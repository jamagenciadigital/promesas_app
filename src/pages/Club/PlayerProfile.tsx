  ...
  if (!player) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-[#0f172a] p-4 text-center">
        ...
        <Button onClick={() => navigate(-1)} className="mt-4 bg-club-primary text-black">Regresar</Button>
      </div>
    );
  }
      ...
      <div className="bg-white dark:bg-[#1e293b] border-b border-gray-200 dark:border-white/5 py-4 sticky top-0 z-50 print:hidden">
        <div className="max-w-5xl mx-auto px-4 flex items-center justify-between">
          ...
          <div className="flex gap-3">
             <Button 
               onClick={handlePrint}
               className="bg-black text-club-primary border-0 px-6 rounded-2xl flex items-center gap-2 uppercase font-black italic text-xs h-12"
             >
          ...
          <div className="p-8 md:p-12 flex flex-col md:flex-row md:flex-wrap lg:flex-nowrap items-center gap-10 relative z-10">
            ...
            <div className="flex-1 text-center md:text-left space-y-4">
              ...
              <h1 className="text-5xl md:text-7xl font-black text-white italic uppercase leading-none tracking-tighter">
                ...
                <span className="text-club-primary">{player.apellidos} {player.segundo_apellido || ''}</span>
              </h1>
              ...
              <div className="pt-4 flex flex-wrap justify-center md:justify-start gap-6 text-white/60 text-sm font-bold uppercase tracking-widest italic">
                <span className="flex items-center gap-2"><Calendar size={18} className="text-club-primary" /> {player.fecha_nacimiento}</span>
                <span className="flex items-center gap-2"><Shield size={18} className="text-club-primary" /> {player.eps}</span>
                <span className="flex items-center gap-2"><MapPin size={18} className="text-club-primary" /> {player.municipio || '---'}</span>
                {player.dorsal && <span className="flex items-center gap-2 font-black text-club-primary"><Trophy size={18} /> DORSAL: {player.dorsal}</span>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 print:grid-cols-2">
          ...
          <div className="space-y-8">
            <section className="bg-white dark:bg-[#1e293b] p-8 rounded-[40px] shadow-sm border border-gray-100 dark:border-white/5 space-y-6 print:border-gray-200">
              <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-club-primary italic border-b border-gray-100 dark:border-white/5 pb-3 print:text-black print:border-gray-100">Información de Identidad</h3>
            ...
            <section className="bg-black text-white p-8 rounded-[40px] shadow-sm space-y-6 print:bg-black print:text-white">
              <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-club-primary italic border-b border-white/10 pb-3">Perfil Deportivo y Antropométrico</h3>
              <div className="grid grid-cols-2 gap-y-4 gap-x-2">
                ...
                {isElite && player.salario && (
                  <div className="col-span-2 pt-2 border-t border-white/10 mt-2">
                    <p className="text-[9px] font-black text-club-primary uppercase tracking-widest mb-1">Salario / Honorarios</p>
                    <p className="text-xl font-black text-white italic leading-none transition-all hover:text-club-primary">
            ...
            <section className="bg-white dark:bg-[#1e293b] p-8 rounded-[40px] shadow-sm border border-gray-100 dark:border-white/5 space-y-6 print:border-gray-200">
              <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-club-primary italic border-b border-gray-100 dark:border-white/5 pb-3 print:text-black print:border-gray-100">Contacto Directo</h3>
          ...
          <div className="space-y-8">
            ...
            <section className="bg-white dark:bg-[#1e293b] p-8 rounded-[40px] shadow-sm border border-gray-100 dark:border-white/5 space-y-6 print:border-red-100">
              <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-club-primary italic border-b border-gray-100 dark:border-white/5 pb-3">Trayectoria Deportiva</h3>
                ...
                {player.trayectorias && player.trayectorias.length > 0 ? (
                  player.trayectorias
                    ...
                    <div key={idx} className="relative pl-8">
                      <div className={`absolute left-0 top-1.5 w-5 h-5 rounded-full border-2 ${tray.es_actual ? 'bg-club-primary border-club-primary' : 'bg-white dark:bg-[#1e293b] border-gray-300 dark:border-white/20'}`}></div>
                      <div>
            ...
            {!isElite && (
              <section className="bg-white dark:bg-[#1e293b] p-8 rounded-[40px] shadow-sm border border-gray-100 dark:border-white/5 space-y-6 print:hidden">
                {(player.equipo2 || player.equipo3) && (
                  <div className="space-y-4">
                    <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-club-primary italic border-b border-gray-100 dark:border-white/5 pb-3 print:text-black">Equipos Adicionales (Ascendido)</h3>
                    <div className="flex flex-col gap-3">
                      {player.equipo2 && (
                        <div className="flex items-center gap-3 p-3 rounded-2xl bg-gray-900 text-white font-black uppercase italic text-[11px] tracking-tight border border-club-primary/20">
                          <Trophy size={14} className="text-club-primary" /> {player.equipo2.nombre}
                        </div>
                      )}
                      {player.equipo3 && (
                        <div className="flex items-center gap-3 p-3 rounded-2xl bg-gray-900 text-white font-black uppercase italic text-[11px] tracking-tight border border-club-primary/20">
                          <Trophy size={14} className="text-club-primary" /> {player.equipo3.nombre}
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {[
                    ...
                    <div key={idx} className="flex items-center justify-between p-4 rounded-2xl bg-gray-50 dark:bg-white/5 group">
                      <div className="flex items-center gap-3">
                        <FileText className="text-gray-400 group-hover:text-club-primary transition-colors" size={18} />
                        <span className="text-[10px] font-black uppercase text-gray-500 dark:text-gray-400">{doc.label}</span>
                      </div>
                      {doc.url ? (
                        <a href={doc.url} target="_blank" rel="noreferrer" className="text-black dark:text-club-primary hover:scale-110 transition-transform"><Download size={18} /></a>
                      ) : (
