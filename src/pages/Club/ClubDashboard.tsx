...
export default function ClubDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [teamsCount, playersCount, coachesCount, walletData, clubInfo, clubPlans] = await Promise.all([
        supabase.from('equipos').select('*', { count: 'exact', head: true }).eq('club_id', activeClubId),
        supabase.from('deportistas').select('*', { count: 'exact', head: true }).eq('club_id', activeClubId),
        supabase.from('perfiles').select('*', { count: 'exact', head: true }).eq('club_id', activeClubId).eq('rol', 'entrenador'),
        ...
        supabase.from('planes_club').select('*', { count: 'exact', head: true }).eq('club_id', activeClubId)
      ]);
