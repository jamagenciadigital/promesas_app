        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {sessions.map(session => (
            <div 
              ...
              className="bg-white dark:bg-[#1e293b]/40 border border-gray-100 dark:border-white/5 rounded-[40px] p-8 hover:border-[var(--primary-50)] transition-all cursor-pointer group"
            >
              <div className="flex justify-between items-start mb-6">
                ...
                {session.completada && (
                  <div className="bg-[var(--primary)] text-black text-[8px] font-black px-2 py-1 rounded-full uppercase italic">
                    {t('calendar.pres')}
                  </div>
                )}
              </div>
              <h3 className="text-xl font-black text-gray-900 dark:text-white uppercase italic mb-2 truncate group-hover:text-[var(--primary)] transition-colors">
                {session.titulo}
              </h3>
              <div className="flex items-center gap-2 text-xs text-gray-500 font-bold mb-6">
                 {session.equipo?.nombre && (
                   <>
                    <span className="text-[var(--primary)]">{session.equipo.nombre}</span>
                    <span>•</span>
                   </>
                 )}
        ...
        <form onSubmit={handleSave} className="space-y-8 py-4 px-2 overflow-x-hidden custom-scrollbar max-h-[80vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <select 
                required
                disabled={form.completada}
                className="w-full h-14 bg-gray-50 dark:bg-white/5 border-none rounded-2xl px-6 text-sm font-bold appearance-none focus:ring-2 focus:ring-[var(--primary)] dark:text-white disabled:opacity-50"
              ...
              {availableDates.length > 0 ? (
                <div className="relative group">
                  <select 
                    required
                    disabled={form.completada}
                    className="w-full h-14 bg-[var(--primary-5)] border-2 border-[var(--primary-20)] rounded-2xl px-6 text-sm font-black appearance-none focus:ring-2 focus:ring-[var(--primary)] dark:text-white transition-all hover:border-[var(--primary-50)]"
                    value={form.fecha}
                    onChange={async (e) => {
                  ...
                  <div className="absolute right-6 top-1/2 -translate-y-1/2 pointer-events-none text-[var(--primary)]">
                ...
                <input 
                  ...
                  className="w-full h-14 bg-gray-50 dark:bg-white/5 border-none rounded-2xl px-6 text-sm font-bold focus:ring-2 focus:ring-[var(--primary)] dark:text-white disabled:opacity-50"
                />
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="space-y-4">
              ...
              <div className="relative group">
                <input 
                  ...
                  className="w-full h-14 bg-gray-50 dark:bg-white/5 border-none rounded-2xl px-6 text-sm font-black focus:ring-2 focus:ring-[var(--primary)] dark:text-white cursor-default"
                />
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center h-14 bg-[var(--primary-5)] rounded-2xl px-6 border border-[var(--primary-10)]">
                <span className="text-sm font-black text-[var(--primary)] italic uppercase tracking-widest">
            ...
            <div className="space-y-4">
              ...
              <input 
                ...
                className="w-full h-14 bg-gray-50 dark:bg-white/5 border-none rounded-2xl px-6 text-sm font-black focus:ring-2 focus:ring-[var(--primary)] dark:text-white disabled:opacity-50"
              />
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <textarea 
              ...
              className="w-full p-6 bg-gray-50 dark:bg-white/5 border-none rounded-[32px] text-sm font-medium focus:ring-2 focus:ring-[var(--primary)] dark:text-white min-h-[100px] disabled:opacity-50"
            />
          </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-white/5 pb-4">
              ...
              {!form.completada && (
                <Button type="button" onClick={handleAddObjective} variant="ghost" className="text-[var(--primary)] hover:bg-[var(--primary-5)] gap-2 font-black italic uppercase text-[10px] tracking-widest">
                ...
                <div key={index} className="flex gap-4 animate-in slide-in-from-left-2 duration-300 items-center">
                  ...
                  <div className="flex-1 flex items-center gap-3 bg-gray-50 dark:bg-white/5 rounded-xl px-4 h-12">
                    ...
                    {new Date(form.fecha).toDateString() === new Date().toDateString() && !form.completada && (
                      <button
                        ...
                        className={cn(
                          ...
                          obj.met ? "bg-[var(--primary)] text-black" : "bg-gray-200 dark:bg-white/10 text-gray-400"
                        )}
                      >
                        <Check size={14} />
                      </button>
                    ...
                    {form.completada && obj.met && (
                      <div className="bg-[var(--primary)] text-black p-1 rounded-lg">
                        <Check size={14} />
                      </div>
                    )}
                  </div>
            ...
            {(['fase_calentamiento', 'fase_principal', 'fase_vuelta_calma'] as const).map(phaseKey => (
              <div key={phaseKey} className="bg-white dark:bg-[#1e293b]/20 border border-gray-100 dark:border-white/5 rounded-[32px] p-8 space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  {!form.completada && (
                    <Button 
                      ...
                      className="h-9 rounded-xl bg-[var(--primary)] text-black hover:bg-[var(--primary)]/90 text-[9px] font-black uppercase tracking-widest gap-1 border-none shadow-lg shadow-[var(--primary-10)]"
                    >
                ...
                <textarea 
                  ...
                  className="w-full p-4 bg-white dark:bg-[#1e293b]/20 border-none rounded-2xl text-xs font-medium focus:ring-1 focus:ring-[var(--primary)] dark:text-white min-h-[60px] disabled:opacity-50"
                ...
                <div className="space-y-3">
                  {form[phaseKey].tasks.map((task, idx) => (
                    <div key={idx} className="flex gap-3 items-center group">
                      <div className="w-1 h-10 bg-[var(--primary)] rounded-full shrink-0" />
                      <input 
                        type="text"
                        disabled={form.completada}
                        placeholder={t('planning.add_task')}
                        className="flex-1 bg-white dark:bg-[#1e293b]/20 border-none rounded-xl px-4 h-10 text-xs font-bold dark:text-white focus:ring-1 focus:ring-[var(--primary)] disabled:opacity-50"
                        value={task}
          ...
          <div className="space-y-4">
            ...
            <textarea 
              ...
              className={cn(
                ...
                !isNotesActive ? "opacity-30 cursor-not-allowed italic" : "focus:ring-2 focus:ring-[var(--primary)]"
              )}
            ...
            {isNotesActive && (
              <div className="flex items-center gap-3 p-4 bg-[var(--primary-5)] rounded-2xl border border-[var(--primary-10)]">
                ...
                <p className="text-[10px] text-gray-600 dark:text-gray-400 font-bold uppercase tracking-wide">
                  {t('planning.complete_session')} <span className="text-[var(--primary)]">(Nota requerida)</span>
                </p>
                <div className="ml-auto">
                   <input 
                    ...
                    className="w-5 h-5 rounded-lg border-none bg-white dark:bg-white/10 text-[var(--primary)] focus:ring-[var(--primary)]"
                   />
                </div>
              </div>
            )}
          ...
          {form.completada && (
            <div className="pt-8 text-center text-[10px] font-black uppercase tracking-widest text-[var(--primary)] italic">
