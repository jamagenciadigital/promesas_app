  ...
  if (!profile?.club_id) {
    return (
      <div className="p-12 text-center flex flex-col items-center justify-center">
        ...
        <h3 className="text-xl font-black text-gray-900 uppercase italic">Perímetro no configurado</h3>
  ...
  return (
    <div className="p-[1.2rem] space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-xl font-black text-gray-900 uppercase italic tracking-tighter">Configuración del Club</h2>
      </div>

      {successMsg && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-start gap-3">
          {syncing ? <RefreshCw className="w-5 h-5 text-emerald-500 animate-spin shrink-0 mt-0.5" /> : <CheckCircle2 className="w-5 h-5 text-emerald-500 shrink-0 mt-0.5" />}
          <p className="text-sm font-bold text-gray-900 uppercase italic">{successMsg}</p>
        </div>
      )}

      {error && <div className="bg-red-50 border border-red-100 rounded-xl p-3 text-red-600 text-xs font-bold italic">{error}</div>}

      <form onSubmit={handleSave} className="space-y-10 pb-20">
        <section className="border-t border-gray-100 pt-6 space-y-6">
          <div className="flex items-center gap-2">
            <CalendarDays className="text-[#182332]" size={20} />
            <h3 className="text-sm font-bold text-[#182332] uppercase tracking-wider">Vigencia de la Temporada</h3>
          </div>
          <p className="text-xs font-semibold text-gray-500 tracking-wider">Nota: Si cambias estas fechas, el sistema sincronizará automáticamente la cartera de todos los jugadores activos.</p>
        ...
        {/* Identidad Visual */}
        <section className="border-t border-gray-100 pt-6 space-y-6">
          <h3 className="text-sm font-bold text-[#182332] uppercase tracking-wider">Identidad Visual</h3>
            ...
            <div className="flex-1 w-full space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div className="space-y-1.5">
                  <label className="block text-xs font-semibold text-gray-700 mb-1.5">Color Principal</label>
                  <div className="flex gap-4 items-center">
                    ...
                    />
                    <span className="text-sm font-bold font-mono text-gray-900">{formData.color_principal.toUpperCase()}</span>
                  </div>
                </div>
              </div>
              
              <div className="space-y-2">
                <label className="block text-xs font-semibold text-gray-700 mb-1.5">Descripción / Slogan</label>
                <textarea 
                  name="descripcion"
                  value={formData.descripcion}
                  onChange={(e) => handleChange(e as any)}
                  className="w-full h-12 bg-gray-50 border border-gray-200 rounded-xl px-4 text-sm font-medium text-gray-900 placeholder-gray-400 outline-none focus:ring-2 focus:ring-club-primary focus:border-transparent transition-all min-h-[100px] py-3 resize-none"
        ...
        <section className="border-t border-gray-100 pt-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <Input label="Email Corporativo" type="email" name="email_corporativo" value={formData.email_corporativo} onChange={(e) => handleChange(e as any)} className="w-full h-12 bg-gray-50 border border-gray-200 rounded-xl px-4 text-sm font-medium text-gray-900 placeholder-gray-400 outline-none focus:ring-2 focus:ring-club-primary focus:border-transparent transition-all" />
            <Input label="Teléfono / WhatsApp" name="telefono" value={formData.telefono} onChange={(e) => handleChange(e as any)} className="w-full h-12 bg-gray-50 border border-gray-200 rounded-xl px-4 text-sm font-medium text-gray-900 placeholder-gray-400 outline-none focus:ring-2 focus:ring-club-primary focus:border-transparent transition-all" />
            <Input label="Sitio Web" name="website" value={formData.website} onChange={(e) => handleChange(e as any)} className="w-full h-12 bg-gray-50 border border-gray-200 rounded-xl px-4 text-sm font-medium text-gray-900 placeholder-gray-400 outline-none focus:ring-2 focus:ring-club-primary focus:border-transparent transition-all" />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <Input label="País" name="pais" value={formData.pais} onChange={(e) => handleChange(e as any)} className="w-full h-12 bg-gray-50 border border-gray-200 rounded-xl px-4 text-sm font-medium text-gray-900 placeholder-gray-400 outline-none focus:ring-2 focus:ring-club-primary focus:border-transparent transition-all" />
            <Input label="Ciudad" name="ciudad" value={formData.ciudad} onChange={(e) => handleChange(e as any)} className="w-full h-12 bg-gray-50 border border-gray-200 rounded-xl px-4 text-sm font-medium text-gray-900 placeholder-gray-400 outline-none focus:ring-2 focus:ring-club-primary focus:border-transparent transition-all" />
            <Input label="Dirección / Sede Principal" name="direccion" value={formData.direccion} onChange={(e) => handleChange(e as any)} className="w-full h-12 bg-gray-50 border border-gray-200 rounded-xl px-4 text-sm font-medium text-gray-900 placeholder-gray-400 outline-none focus:ring-2 focus:ring-club-primary focus:border-transparent transition-all" />
          </div>
        </section>


        <div className="flex justify-end pt-8">
          <Button type="submit" isLoading={saving || syncing} className="px-12 h-14 bg-black text-white font-bold rounded-xl shadow-sm hover:bg-black/90 transition-all">
