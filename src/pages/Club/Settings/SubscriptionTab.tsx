    ...
    <div className="p-[1.2rem] max-w-5xl mx-auto space-y-6 animate-in fade-in pb-24">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-club-primary/10 rounded-lg">
            <ShieldCheck className="w-6 h-6 text-club-primary" />
          </div>
          <div>
            <h2 className="text-2xl font-bold text-[#182332] tracking-tight">Suscripción y Planes</h2>
            <p className="text-sm text-gray-400 mt-1">Gestiona los planes que ofreces a tus miembros.</p>
          </div>
        </div>
        
        <Button 
          onClick={() => handleOpenModal()}
          className="bg-black text-white font-bold rounded-xl shadow-sm hover:bg-black/90 transition-all px-6 py-2.5 flex items-center gap-2 border-0"
        >
      ...
      {successMsg && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-start gap-3 animate-in slide-in-from-top-2">
          ...
          <div className="relative z-10 flex flex-col xl:flex-row xl:items-center justify-between gap-8">
            <div className="space-y-4 flex-1">
              <div className="flex items-center gap-2">
                <span className="bg-club-primary/10 text-club-primary text-[8px] font-black uppercase tracking-widest px-3 py-1 rounded-full border border-club-primary/20 italic">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 pt-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xl font-bold text-club-primary italic">{b2bPlan.limite_jugadores === -1 ? 'Ilimitado' : b2bPlan.limite_jugadores}</p>
                </div>
              </div>
            </div>
            ...
            <div className="shrink-0 text-center xl:text-right border-t xl:border-t-0 xl:border-l border-white/10 pt-6 xl:pt-0 xl:pl-10">
              ...
              <div className="flex items-end justify-center xl:justify-end gap-1.5">
                <span className="text-4xl font-black text-club-primary italic leading-none">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {plans.length > 0 ? (
          plans.map((plan) => (
            <div 
              key={plan.id}
              className={`bg-white rounded-2xl border p-6 relative group hover:border-club-primary transition-all shadow-sm hover:shadow-lg ${plan.periodo === 'Único' ? 'border-club-primary/40 ring-1 ring-club-primary/10' : 'border-gray-100'}`}
            >
              <div className="flex justify-between items-start mb-4">
                <div className={`p-2 rounded-lg ${plan.periodo === 'Único' ? 'bg-club-primary/10 text-club-primary' : 'bg-gray-50 text-gray-400'}`}>
              ...
              <div className="flex items-center gap-2 text-sm text-gray-500 mb-4 font-medium uppercase tracking-widest text-[10px]">
                {plan.periodo === 'Único' ? (
                   <span className="flex items-center gap-1.5 text-club-primary bg-club-primary/10 px-2 py-0.5 rounded-md">
              ...
              <div className="pt-4 border-t border-gray-50 flex items-end gap-1">
          ...
          <div className="col-span-full py-20 bg-gray-50/50 border-2 border-dashed border-gray-200 rounded-[32px] flex flex-col items-center justify-center text-center px-6">
            <Package className="w-12 h-12 text-gray-300 mb-3" />
            <p className="text-gray-500 font-medium">No has creado ningún plan todavía.</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-gray-100">
              <h3 className="text-xl font-bold text-gray-900 font-outfit uppercase tracking-tight">
            ...
            <form onSubmit={handleSavePlan} className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                <label className="block text-sm font-medium text-gray-700 mb-1.5 font-bold uppercase tracking-widest text-[10px] opacity-60">Tipo de Cobro / Periodo</label>
                    ...
                    <button
                      ...
                      className={`flex items-center justify-between px-5 py-4 rounded-xl text-sm font-bold border-2 transition-all ${
                        planForm.periodo === p.value
                          ? 'bg-club-primary/10 border-club-primary text-gray-900'
                          : 'bg-gray-50 border-transparent text-gray-500 hover:border-gray-200'
                      }`}
                    >
                      <span>{p.label}</span>
                      {p.value === 'Único' && <Wallet size={16} className="text-club-primary" />}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <Button
                  type="submit"
                  isLoading={saving}
                  className="flex-1 bg-black text-club-primary font-black uppercase tracking-widest text-xs h-12 rounded-xl"
                >
