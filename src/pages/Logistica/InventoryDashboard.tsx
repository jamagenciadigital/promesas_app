    ...
    <div className="p-4 md:p-8 space-y-6 md:space-y-10 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div className="flex items-center gap-4 md:gap-6">
          <div className="w-14 h-14 md:w-16 md:h-16 bg-[var(--primary)] rounded-[20px] md:rounded-[24px] flex items-center justify-center shadow-lg shadow-[var(--primary-20)] rotate-3 transition-transform group-hover:rotate-0">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
          {isEscenario && scenarios.length > 1 && (
            <select 
              ...
              className="w-full sm:w-auto h-14 px-6 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase italic tracking-widest text-white outline-none focus:border-[var(--primary-40)] transition-all"
            >
          ...
          <Button 
            ...
            className="w-full sm:w-auto h-14 px-8 bg-[var(--primary)] text-black font-black uppercase italic tracking-widest text-xs rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-xl shadow-[var(--primary-10)]"
          >
      ...
      <div className="flex flex-col md:flex-row gap-4 bg-white/5 p-3 md:p-4 rounded-[32px] border border-white/5">
        <div className="relative flex-1 group">
          <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-[var(--primary)] transition-colors" size={20} />
          <input 
            type="text"
            placeholder="Buscar por nombre o descripción..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full h-14 md:h-16 bg-black/40 border border-transparent focus:border-[var(--primary-20)] rounded-[20px] md:rounded-[24px] pl-16 pr-6 text-sm text-white outline-none transition-all placeholder:text-gray-600"
          />
        </div>
        <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
          <button 
            onClick={() => setCategoryFilter('all')}
            className={`h-14 md:h-16 px-6 md:px-8 rounded-[20px] md:rounded-[24px] text-[9px] md:text-[10px] font-black uppercase italic tracking-widest transition-all whitespace-nowrap ${categoryFilter === 'all' ? 'bg-[var(--primary)] text-black' : 'bg-black/40 text-gray-500 hover:text-white border border-white/5'}`}
          >
            Todos
          </button>
          {CATEGORIES.map(cat => (
            <button 
              ...
              className={`h-14 md:h-16 px-6 md:px-8 rounded-[20px] md:rounded-[24px] text-[9px] md:text-[10px] font-black uppercase italic tracking-widest transition-all whitespace-nowrap flex items-center gap-3 ${categoryFilter === cat.id ? 'bg-[var(--primary)] text-black' : 'bg-black/40 text-gray-500 hover:text-white border border-white/5'}`}
            >
              <span>{cat.icon}</span>
              {cat.label}
            </button>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {filteredItems.map((item) => (
            <div key={item.id} className="group bg-white/5 border border-white/5 rounded-[40px] md:rounded-[48px] overflow-hidden hover:border-[var(--primary-30)] transition-all duration-500 shadow-sm hover:shadow-2xl hover:-translate-y-1">
              <div className="p-6 md:p-8 space-y-6">
                <div className="flex justify-between items-start">
                  <div className="space-y-2">
                    ...
                    <h3 className="text-lg md:text-xl font-black text-white uppercase italic tracking-tighter leading-none group-hover:text-[var(--primary)] transition-colors">
                      {item.nombre}
                    </h3>
                  </div>
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleOpenModal(item)} className="p-2.5 bg-black/60 rounded-xl md:rounded-2xl text-gray-400 hover:text-[var(--primary)] transition-colors"><Edit2 size={14} /></button>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className={`p-4 rounded-2xl md:rounded-3xl border space-y-1 text-center ${item.cantidad_disponible <= 2 ? 'bg-red-500/10 border-red-500/20' : 'bg-[var(--primary-10)] border-[var(--primary-20)]'}`}>
                    <p className="text-[8px] font-black text-gray-600 uppercase tracking-widest">En Stock</p>
                    <p className={`text-xl md:text-2xl font-black italic ${item.cantidad_disponible <= 2 ? 'text-red-500' : 'text-[var(--primary)]'}`}>
        ...
        <form onSubmit={handleSubmit} className="p-2 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar pr-4">
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <select 
                ...
                className="w-full h-14 md:h-16 px-6 bg-black/40 rounded-2xl text-sm font-bold text-white outline-none border border-transparent focus:border-[var(--primary-40)] transition-all appearance-none"
              >
            ...
            <div className="space-y-2">
              ...
              <textarea 
                ...
                className="w-full bg-black/40 border border-transparent focus:border-[var(--primary-20)] rounded-2xl p-4 md:p-6 text-sm text-white outline-none min-h-[100px] resize-none transition-all"
              />
            </div>
          </div>
          ...
          <div className="pt-6 flex gap-4">
            ...
            <Button 
              type="submit"
              isLoading={saving}
              className="flex-[2] h-14 bg-[var(--primary)] text-black font-black uppercase italic tracking-widest text-xs rounded-2xl shadow-xl shadow-[var(--primary-10)]"
            >
